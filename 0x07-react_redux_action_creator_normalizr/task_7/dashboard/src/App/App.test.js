...
describe("rendering components", () => {
  ...
  it('should render Notifications component', () => {
    const wrapper = shallow(<App />); // Provide props

    expect(wrapper.find(Notifications)).toHaveLength(1); // Check for a class
  });
...
describe("When isLogged in is true", () => {
  ...
  it(`Test that the logIn function updates user's state correctly`, () => {
    const wrapper = mount(
  ...
  it(`Test that the logOut function updates user's state correctly`, () => {
    const wrapper = mount(
      <AppContext.Provider value={{ user, logOut }}>
        <App />
      </AppContext.Provider>
